    ...
    def train(self, trainloader, net, epoch):
        ...
        for e in range(epoch):
            ...
            for _, data, labels in tqdm(trainloader, desc=f"Epoch {e+1}/{epoch}", total=len(trainloader)):
                ...
                loss_sum += np.mean(loss.cpu().detach().numpy()).item()
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
if __name__ == "__main__":
    torch.backends.cudnn.deterministic = True  #cudnn是GPU加速的用于深度神经网络的原语库
    torch.backends.cudnn.benchmark = True    #复现、算法选择
    np.random.seed(conf.seed)          #利用随机数种子，使得每次生成的随机数相同
    train_task = TrainQualityTask(conf)    #把定义的训练任务加载给train_task
    torch.manual_seed(conf.seed)         # 设置 CPU 生成随机数的 种子 ，方便下次复现实验结果
