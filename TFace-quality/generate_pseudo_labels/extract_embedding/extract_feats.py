...
if __name__ == "__main__":
    ...
    feats = np.zeros([sample_num, conf.embedding_size])               #这个512是怎么确定的？              # initnte features of all samples
    with torch.no_grad():
        for datapath, data in tqdm(dataloader, total=len(dataloader)):
            data = data.to(device)
            embeddings = F.normalize(net(data), p=2, dim=1).cpu().numpy().squeeze()  #cpu()把数据处理设备拿到cpu上 #squeeze()去掉1的维度
...
'''Number of samples: 5908
/usr/local/lib/python3.7/dist-packages/torch/utils/data/dataloader.py:490: UserWarning: This DataLoader will create 4 worker processes in total. Our suggested max number of worker in current system is 2, which is smaller than what this DataLoader is going to create. Please be aware that excessive worker creation might get DataLoader running slow or even freeze, lower the worker number to avoid potential slowness/freeze if necessary.
  cpuset_checked))
Sample_num = 5908
100% 3/3 [11:05<00:00, 221.70s/it]
Traceback (most recent call last):
  File "extract_feats.py", line 102, in <module>
    np.save(outfile, feats)
  File "<__array_function__ internals>", line 6, in save
  File "/usr/local/lib/python3.7/dist-packages/numpy/lib/npyio.py", line 525, in save
    file_ctx = open(file, "wb")
FileNotFoundError: [Errno 2] No such file or directory: '../feats_npy/Embedding_Features.npy'
'''